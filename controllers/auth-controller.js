...
const createSendToken = (user, statusCode, res) => {
    ...
    // only send cookie on an encrypted connection (https)
...
exports.protect = catchAsync( async (req, res, next) => {
    ...
    //4. Check if user changed password after the token was issued
        // if the user changed password after the token was issued, we don't want to grant access
...
exports.forgotPassword = catchAsync(async(req, res, next) => {
    ...
    await user.save({ validateBeforeSave: false }); // save the document without validating it
...
exports.resetPassword = catchAsync(async(req, res, next) => {
    // 1. Get user based on the token
        // in database, we have the encrypted token, so we need to encrypt the token from the request body and compare them
    ...
    user.passwordResetExpires = undefined; 
    await user.save(); // save the document with validation

    // 3. Update changedPasswordAt property for the user
        // this is done in the pre save middleware in userModel.js
...
exports.updatePassword = catchAsync(async(req, res, next) => {
    ...
    const user = await User.findById(req.user.id).select('+password'); // we need to select the password because it is hidden by default
